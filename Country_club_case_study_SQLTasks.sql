...
FROM Facilities
WHERE membercost < (monthlymaintenance *.2) and
      membercost > 0
...
left join Facilities as f
     ON b.facid = f.facid
where b.starttime like '2012-09-14%' and (CASE WHEN m.memid = 0 Then (b.slots * f.guestcost)
            Else (b.slots * f.membercost) END) > 30
order by Cost desc

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

select sub.member_name, sub.name,sub.cost
from (select concat_ws(" ",m.firstname,m.surname) as member_name, f.name,b.starttime,
       CASE WHEN m.memid = 0 Then (b.slots * f.guestcost)
            Else (b.slots * f.membercost) END as cost
      from Members as m
        join Bookings as b
...
select m.surname,m.firstname,m.recommendedby,r.surname,r.firstname
FROM Members m
JOIN Members r
   ON m.recommendedby = r.memid
Where m.memid > 0 and m.recommendedby > 0
...
SELECT f.name AS Facility,month(starttime) as Month,
	count(b.bookid) AS member_usage
FROM Facilities AS f
JOIN Bookings AS b ON b.facid=f.facid
Where b.memid > 0
